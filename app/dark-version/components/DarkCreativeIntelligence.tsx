  ...
  return (
    <section className="w-full px-6 py-12 sm:px-8 lg:px-12">
      <div className="mx-auto max-w-4xl text-center">
        ...
        <p className="mx-auto mt-3 max-w-xl font-prompt font-normal text-[15px] leading-relaxed text-[#DDDDDD]">{description}</p>
      </div>
      ...
      <div ref={scrollRef} className="mt-10 flex gap-3 overflow-x-auto pb-2 [scrollbar-width:none]">
        {cards.map((card) => (
          ...
          >
            ...
            <div>
              <h3 className="text-lg font-prompt font-normal uppercase text-[#DDDDDD]">{card.title}</h3>
              <p className="mt-1 text-xs font-prompt font-normal leading-relaxed text-[#DDDDDD]">{card.description}</p>
            </div>
          </article>
        ))}
      </div>
